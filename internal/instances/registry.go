...
func (r *Registry) Get(ctx context.Context, id string) (*Instance, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, name, workspace, intent, stack, status, token, registered_at, last_seen
		 FROM instances WHERE id = ?`, id).
		Scan(&inst.ID, &inst.Name, &inst.Workspace, &inst.Intent, &inst.Stack, &inst.Status, &inst.Token, &registeredAt, &lastSeen)
	if err != nil {
		return nil, err
	}
...
func (r *Registry) List(ctx context.Context) ([]Summary, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, workspace, intent, stack, status, registered_at, last_seen
	...
	for rows.Next() {
		var item Summary
		var registeredAt, lastSeen string
		if err := rows.Scan(&item.ID, &item.Name, &item.Workspace, &item.Intent, &item.Stack, &item.Status, &registeredAt, &lastSeen); err != nil {
...
func (r *Registry) Discover(ctx context.Context, name, workspace, stack string) ([]Summary, error) {
	query := `SELECT id, name, workspace, intent, stack, status, registered_at, last_seen FROM instances WHERE 1=1`
	args := []any{}
